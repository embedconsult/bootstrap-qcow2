...
module Bootstrap
  ...
  module Tarball
    # Write a gzip-compressed tarball containing *source*.
    def self.write_gz(source : Path, output : Path, base_path : Path? = nil) : Nil
      root = base_path || source
      TarWriter.write_gz([source], output, root)
    end
