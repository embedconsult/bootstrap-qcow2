...
module Bootstrap
  ...
  # * aarch64-first: default minirootfs/branch target the Alpine aarch64 release.
  ...
  class SysrootBuilder
    ...
    # Build a PackageSpec pointing at the Alpine minirootfs tarball for the
    # configured architecture and branch. The checksum URL is derived from the
    # upstream naming convention when available.
    ...
    # * extracts the Alpine minirootfs
    # * creates workspace/sources/var/lib directories
    # * copies cached source archives and the coordinator entrypoints
    ...
    # Copy all downloaded source archives into the rootfs for offline builds.
    ...
    # * extracts each source into /workspace
    # * runs a basic configure/bootstrap step
    ...
    # tooling that expects a chroot-able Alpine environment.
