...
module Bootstrap
  ...
  module BuildPlanUtils
    DEFAULT_WORKSPACE_ROOT = SysrootWorkspace::ROOTFS_WORKSPACE_PATH.to_s

    # Returns a copy of *plan* with its workdir root rewritten.
    #
    # This updates the phase workdir, phase destdir (when present), and each
    # step workdir (plus any step-level install prefix/destdir fields) when they
    # are rooted at `from_root`.
    def self.rewrite_workdir(plan : BuildPlan, to_root : String, from_root : String = DEFAULT_WORKSPACE_ROOT) : BuildPlan
      phases = plan.phases.map do |phase|
        ...
        BuildPhase.new(
          ...
          workdir: rewrite_root(phase.workdir, from_root, to_root),
