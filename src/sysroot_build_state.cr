...
module Bootstrap
  ...
  class SysrootBuildState
    ...
    def initialize(@workspace : SysrootWorkspace = SysrootWorkspace.new,
      ...
      # resolved version used for resume decisions. Overrides are read-only here;
      # they are only generated by `sysroot-builder-overrides`.
      apply_overrides
      ...
      update_overrides_tracking(previous_state, current_overrides_digest, invalidate_on_overrides)
    end
    ...
    # Apply overrides to *plan* and return the resolved plan without writing it.
    ...
    # Apply overrides from disk to the loaded build plan without modifying the
    # overrides file. Override generation is handled only by
    # `sysroot-builder-overrides`.
    private def apply_overrides
      return unless overrides_exists?
      ...
      @plan = @overrides.not_nil!.apply(@plan.not_nil!)
    end
