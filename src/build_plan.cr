...
module Bootstrap
  ...
  class BuildPlan
    ...
    def selected_phases(requested : String = "all") : Array(BuildPhase)
      ...
      return @phases if requested == "all" || requested.empty?
      matching = @phases.select { |phase| phase.name == requested }
      raise "Unknown build phase #{requested}" if matching.empty?
      matching
    end

    # Return phases that are valid for the current workspace namespace.
    def phases_for_current_workspace(workspace : SysrootWorkspace) : Array(BuildPhase)
      case workspace.namespace
      when .host?
        @phases
      when .seed?
        @phases.reject { |phase| phase.environment.starts_with?("host-") }
      when .bq2?
        @phases.select { |phase| phase.environment.starts_with?("rootfs-") }
      end
    end
  end
end
