...
The namespace helper in `src/namespace_wrapper.cr` exposes `with_new_namespace`, which wraps steps 1–3 and ensures bind mounts are detached in an `ensure` block so temporary mounts do not linger. A caller is still responsible for the pivot/chroot steps so the rootfs becomes the execution context.
