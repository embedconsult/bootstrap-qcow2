...
describe Bootstrap::CodexNamespace do
  ...
  it "stages a single-file tarball even when the name is not codex" do
    status = Process.run(
      "crystal",
      [
        "eval",
        <<-CR,
          ...
          payload_dir = temp_root / "payload"
          FileUtils.mkdir_p(payload_dir)
          File.write(payload_dir / "codex-aarch64-unknown-linux-gnu", "codex-binary")

          tarball = temp_root / "codex.tar"
          tar_status = Process.run("tar", ["-cf", tarball.to_s, "-C", payload_dir.to_s, "."])
          exit tar_status.exit_code unless tar_status.success?
