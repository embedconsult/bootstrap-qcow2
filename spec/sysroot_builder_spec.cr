...
def with_tempdir(&)
  base = Path.new(ENV["TMPDIR"]? || "/tmp")
  path = base / "sysroot-spec-#{Random::Secure.hex(8)}"
...
describe Bootstrap::SysrootBuilder do
  ...
  it "builds a minirootfs spec for the configured architecture" do
    builder = Bootstrap::SysrootBuilder.new(Path["/tmp/work"], "arm64", "edge")
    spec = builder.minirootfs_spec
    spec.name.should eq "alpine-minirootfs"
  ...
  it "builds a plan for each package" do
    with_tempdir do |dir|
      ...
      plan.first.commands.first.should eq ["tar", "xf", "/sources/pkg-1.0.tar.gz", "-C", "/workspace"]
    end
  end
  ...
  it "returns chroot command when running in dry-run mode" do
    with_tempdir do |dir|
      ...
      args.should eq ["chroot", builder.rootfs_dir.to_s, "crystal", "run", "/usr/local/bin/sysroot_runner_main.cr"]
    end
  end
end
